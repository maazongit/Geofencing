...
<body>
  ...
  <script>
    ...
    function takeAction() {
      ...
      if (optionValue === "Check-in") {
        ...
        $.post(url, {
          ...
          alertify.alert('Message', `${myHTML} Checked-In`, function() {
        ...
        $.post(url, {
          ...
          userLng: userLng,
          address: coordinates.address,
          lastInsertCheckInId: lastInsertCheckInId,
          distance: coordinates.distance
        }, function(success) {
          //console.log(data);
          let myHTML = "<center><i class='fa fa-check-circle' style='color:green;font-size:100px;'></i><center>";
          alertify.alert('Message', `${myHTML} Checked-Out`, function() {
            window.location.reload();
          });
        });
      } //if not check out
    ...
    function logout() {
      ...
      alertify.confirm(`${myHTML} Are You Sure?`, function() {
        window.location.href = "./logout.php";
      });
    }
  </script>
  <script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDynkPyncMwGXk_4pVyp575JkefM6kyNUQ&callback=initMap">
  </script>
</body>
